...
let thisDay = document.querySelector("h2")
...
function runFetch(event) {
    ...
    //Delete cards when button is clicked
    while (cardContain.firstChild) {
        cardContain.removeChild(cardContain.firstChild)
    }

    //save the name of the city either from the search bar or a search history button text value
    ...
    fetch(weatherUrl)
        ...
        .then(function (data) {
            ...
            // Send alert and don't execute code if cityname is not found
            if (data.cod > 200) {
                ...
                thisDay.textContent = data.city.name + "(" + dayjs.unix(allDates[0].dt).format("M/DD/YYYY") + ")"
